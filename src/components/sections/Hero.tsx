import { motion } from 'framer-motion';
import { style } from '../../style';
import CodingIntro from '../ui/CodingIntro';


const Hero = () => {
  return (
    <section className='relative w-full min-h-fit pb-24 mx-auto'>
      <div className={`${style.paddingX} pt-[120px] max-w-7xl mx-auto flex flex-row items-start gap-5`}>
        <div className='flex flex-col justify-center items-center'>
          <div className='w-5 h-5 rounded-full bg-[#4e1c75ee]' />
          <div className='w-1 sm:h-80 h-40 violet-gradient'/>
        </div>
        <div className='w-full'>
          <h1 className={`${style.heroHeadText} text-white`}>
            Hi, I'm <span className='text-[#8758acee]'>Chandan</span>
          </h1>
          <p className={`${style.heroSubText} mt-2 text-white-100`}>
            Welcome to my digital space
          </p>
          <div className="mt-8">
            <CodingIntro />
          </div>
        </div>
      </div>

      <div className='absolute bottom-4 w-full flex justify-center items-center mt-20'>
        <a href="#about">
          <div className='w-[35px] h-[64px] rounded-3xl border-4 border-secondary flex justify-center items-start p-2'>
            <motion.div
              animate={{
                y: [0, 24, 0]
              }}
              transition={{
                duration: 1.5,
                repeat: Infinity,
                repeatType: 'loop'
              }}
              className='w-3 h-3 rounded-full bg-secondary mb-1'
            />
          </div>
        </a>
      </div>
    </section>
  );
};

export default Hero;
